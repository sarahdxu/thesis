...
Overall, Tables B6 and B7 provide evidence that social preference games, even when combined with self-reported measures, have little to no power to explain donations behavior. While this may be a bit discouraging, however, this raises another interesting question: if University Relations wanted to predict which seniors or recent alumni will donate, which tools would be the best for predicting this question? This questions shifts us to now look at predictive modeling. To do so, I performed subset selection on each of my models. Since subset selection chooses which variables are best for predicting the response variable, I included more explanatory variables. I added the GDG parameter \(\rho\), the TG2 average return outcome, and replaced the total SRA score with each of the ten individual SRA questions. 

I first performed best subsets regression for the model using the binary donations representation. Using the lowest Mallows' Cp value to determine the best variables, four of the SRA items were chosen. Table B8 displays the logistic regression model with the chosen variables. {\color{blue}All variables statistically significant.} I then performed best subsets regression for the model including the continuous donations representation. Again using Mallows\rq \ Cp, only the \(\rho\) parameter was chosen. Table B9 reports the tobit regression that includes only \(\rho\). {\color{blue} Not statistically significant}
