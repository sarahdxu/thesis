...
I now turn to the main question of whether the game decisions explain the field behavior. There are two ways that I represented the field measure: the total amount the participant has donated, and (2) whether the participant has donated or not. Table B6 presents five regression analyses, each obtained with Ordinary Least Squares (OLS), that puts together the game results and self-reported measures to show how they predict donation amounts. The first column uses only the game decisions, the second column uses only the total SRA scores, the third column uses only the monetary SRA scores, the fourth column includes both the game decisions and total SRA scores, and the last column includes the game decisions and monetary SRA scores. The models that include only SRA scores (columns 2 and 3) show no predictive power for donations. This is seen by neither the total SRA score nor the monetary SRA score having statistical significance, and the proportions of variance explained by both SRA scores are practically zero. In the models that include the game variables and SRA scores (columns 1, 4, and 5), only the GDG game variable, \(\alpha\), is statistically significant at the 10\% level. This makes sense because although all of the social preference games can be theoretically mapped to donations behavior, \(\alpha\) is the most intuitively related to giving. The proportions of variance explained by the models are low: the models including only the games and both the games and total SRA scores (columns 1 and 4) have a value of 0.10, and the model including both the games and monetary SRA scores (column 5) has a value of 0.11. I then run logistic regressions on the same explanatory variables, but this time using the binary donations representation as the response variable. Table B7 displays the five models with the identical structure as the previous table. This time none of the models' variables have statistical significance. I calculated each models\rq \ McFadden\rq s pseudo \(R_{2}\) values, which is defined as: \\

\( R^{2}_{McF} = 1 - \frac{ln\hat{L}(M_{full})}{ln\hat{L}({M_{intercept}})} \) \\

\noindent where \(\hat{L}\) represents the estimated likelihood, \(M_{full}\) stands for the model with predictors, and \(M_{intercept}\) stands for the model without predictors. Once again, I found that the proportions of variance explained by the regression models are very close to zero. Tables B6 and B7 therefore provide evidence that social preference games, even when combined with the self-reported measures, have little to no power to explain donations behavior.

Another question of interest is whether social preference games, self-reported measures, or both tools are better at predicting the field measures. For each model, I estimated their prediction errors, using mean squared errors (MSE):

\( MSE = \frac{1}{n}\sum_{i=1}^{n}(donations_{i}-\hat{donations}_{i})^{2} \) \\

The bottom row of Tables B6 and B7 report each models\rq s MSE values. For the continuous donations variable (Table B6), the model that has the lowest MSE is the model that includes both the game outcomes and monetary SRA scores. The models that include only the SRA results - total and monetary - have the highest MSE scores. For the binary donations variable, the lowest MSE value belongs to the model with only the game outcomes, and the highest MSE scores are seen with the models with both game outcomes and SRA scores. However, these differences are very small.

All in all,Tables B6 and B7 show that none of the social preference games, even when combined with the SRA results, are statistically significant. The proportion of variance explained by each models are low, with some that are practically zero. In addition, there is only a small difference in MSE scores between the models. All of these results provide evidence that social preference games have little to no power to explain the field behavior, and there does not seem to be a better tool for predicting donations.
...
\cline{2-6} 
\\[-1.8ex] & \multicolumn{5}{c}{Donations} \\ 
\\[-1.8ex] & (1) & (2) & (3) & (4) & (5)\\ 
\hline \\[-1.8ex] 
 Alpha & $-$3.000$^{*}$ &  &  & $-$2.998$^{*}$ & $-$3.001$^{*}$ \\ 
  & (1.772) &  &  & (1.775) & (1.774) \\ 
  & & & & & \\ 
 UG1 & $-$0.885 &  &  & $-$0.887 & $-$0.904 \\ 
  & (2.384) &  &  & (2.388) & (2.388) \\ 
  & & & & & \\ 
 UG2 & 2.108 &  &  & 2.101 & 2.140 \\ 
  & (2.712) &  &  & (2.718) & (2.718) \\ 
  & & & & & \\ 
 TG1 & $-$0.070 &  &  & $-$0.062 & $-$0.066 \\ 
  & (2.014) &  &  & (2.019) & (2.016) \\ 
  & & & & & \\ 
 Reciprocity & $-$0.392 &  &  & $-$0.389 & $-$0.457 \\ 
  & (2.668) &  &  & (2.672) & (2.679) \\ 
  & & & & & \\ 
 PGG & $-$0.553 &  &  & $-$0.549 & $-$0.594 \\ 
  & (1.887) &  &  & (1.890) & (1.893) \\ 
  & & & & & \\ 
 ...
 SRAmoney &  &  & 5.499 &  & 6.575 \\ 
  &  &  & (19.586) &  & (19.857) \\ 
  & & & & & \\ 
 Constant & 305.600 & 139.594 & 30.427 & 332.031 & 238.206 \\ 
  & (224.793) & (338.445) & (224.177) & (398.414) & (303.459) \\ 
  & & & & & \\ 
\hline \\[-1.8ex] 
Observations & 343 & 343 & 343 & 343 & 343 \\ 
R$^{2}$ & 0.010 & 0.0001 & 0.0002 & 0.010 & 0.011 \\ 
Adjusted R$^{2}$ & $-$0.007 & $-$0.003 & $-$0.003 & $-$0.010 & $-$0.010 \\ 
Residual Std. Error & 923.289 (df = 336) & 921.293 (df = 341) & 921.214 (df = 341) & 924.657 (df = 335) & 924.515 (df = 335) \\ 
F Statistic & 0.591 (df = 6; 336) & 0.020 (df = 1; 341) & 0.079 (df = 1; 341) & 0.506 (df = 7; 335) & 0.521 (df = 7; 335) \\ 
MSE & 835065 & 843832 & 843687 & 835048 & 834791 \\
\hline 
\hline \\[-1.8ex] 
...
\hline \\[-1.8ex] 
 Alpha & 0.003 &  &  & 0.003 & 0.003 \\ 
  & (0.004) &  &  & (0.004) & (0.004) \\ 
  & & & & & \\ 
 UG1 & 0.00001 &  &  & 0.0001 & $-$0.0002 \\ 
  & (0.005) &  &  & (0.005) & (0.005) \\ 
  & & & & & \\ 
 UG2 & 0.006 &  &  & 0.006 & 0.006 \\ 
  & (0.006) &  &  & (0.006) & (0.006) \\ 
  & & & & & \\ 
 TG1 & 0.0002 &  &  & $-$0.00004 & 0.0003 \\ 
  & (0.005) &  &  & (0.005) & (0.005) \\ 
  & & & & & \\ 
 Reciprocity & 0.002 &  &  & 0.002 & 0.001 \\ 
  & (0.006) &  &  & (0.006) & (0.006) \\ 
  & & & & & \\ 
 PGG & 0.002 &  &  & 0.001 & 0.001 \\ 
  & (0.004) &  &  & (0.004) & (0.004) \\ 
  & & & & & \\ 
 SRAtotal &  & 0.032 &  & 0.032 &  \\ 
  &  & (0.023) &  & (0.023) &  \\ 
  & & & & & \\ 
 SRAmoney &  &  & 0.071 &  & 0.072 \\ 
  &  &  & (0.045) &  & (0.046) \\ 
  & & & & & \\ 
 Constant & 0.089 & $-$0.475 & $-$0.209 & $-$0.947 & $-$0.645 \\ 
  & (0.506) & (0.771) & (0.511) & (0.902) & (0.687) \\ 
  & & & & & \\ 
\hline \\[-1.8ex] 
Observations & 343 & 343 & 343 & 343 & 343 \\ 
Log Likelihood & $-$223.085 & $-$222.880 & $-$222.580 & $-$222.114 & $-$221.815 \\ 
Akaike Inf. Crit. & 460.169 & 449.760 & 449.159 & 460.228 & 459.630 \\ 
MSE & 4.3732 & 4.3794 & 4.3874 & 4.4125 & 4.4170 \\
\hline 
\hline \\[-1.8ex] 
\textit{Note:}  & \multicolumn{5}{r}{$^{*}$p$<$0.1; $^{**}$p$<$0.05; $^{***}$p$<$0.01} \\ 
\end{tabular} 
